	...
	public static int maxProduct(String[] words) {
		...
		for (int i = 0; i < words.length; i++) {
			for (char s : words[i].toCharArray()) {
				mask[i] |= 1 << (s - 'a'); // 'a' - 'a' = 0, left romve 0 position. every charcater has its flag, 0
											// means not exist, 1 means exit;
			}
		}
		int max = 0;
		for (int i = 0; i < mask.length; i++) {
			if (words[i].length() * words[i].length() <= max)
				break; // words is sorted with order of descending 5 4 3 2 1
