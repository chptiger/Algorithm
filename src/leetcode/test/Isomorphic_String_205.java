...
public class Isomorphic_String_205 {
	public static void main(String[] args) {
		...
		boolean flag = isIsomorphic_int_2(s1, s2);
	 ...
	 */
	public static boolean isIsomorphic_int_1(String s1, String s2) {
		int[] m = new int[512];// default value is 0
		for (int i = 0; i < s1.length(); i++) {
			// if (m[s1.charAt(i)] != m[s2.charAt(i)+256]) return false;// i == 0, default
			// value is 0 == 0
		...
		for (int i = 0; i < s1.length(); i++) {
			if (m[s1.charAt(i)] != m[s2.charAt(i) + 256])
				return false;// i == 0, default value is 0 == 0
			// m[s1.charAt(i)] = m[s2.charAt(i)+256] = i+1;
			// System.out.println(m[s1.charAt(i)]);
		}
		return true;
	...
	// 把相同的位置的字符存到数组里，存入值是index+1. 字符重复的位置，应该存入相同数值，不对应的时候，就返回false了，
	public static boolean isIsomorphic_int_2(String s1, String s2) {
		int[] m = new int[512];// default value is 0
		for (int i = 0; i < s1.length(); i++) {
			if (m[s1.charAt(i)] != m[s2.charAt(i) + 256])
				return false;//
	 ...
	 * Time Limit Exceeded
	 * 
	 * public static boolean isIsmophic(String s1, String s2){ if(s1 == null || s2
	 * == null || s1.length()!= s2.length()) return false;
	 * 
	 * for(int i = 0; i< s1.length(); i++){ for(int j = i+1; j< s2.length(); j++){
