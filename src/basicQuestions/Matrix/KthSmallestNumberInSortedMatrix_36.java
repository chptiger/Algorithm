 ...
 * 
 * Examples { {1, 3, 5, 7}, {2, 4, 8, 9}, {3, 5, 11, 15}, {6, 8, 13, 18} } the
 * 5th smallest number is 4 the 8th smallest number is 6
 *
 */
public class KthSmallestNumberInSortedMatrix_36 {

	public static void main(String[] args) {
		int[][] matrix = new int[][] { { 1, 1, 1, 1 }, { 1, 1, 2, 9 }, { 2, 5, 11, 15 }, { 6, 8, 13, 18 } };
	...
	public static int findIthSmallestNumber(int[][] matrix, int index) {
		...
		PriorityQueue<Cell> queue = new PriorityQueue<Cell>(index, new Comparator<Cell>() {
		...
		for (int i = 0; i < index - 1; i++) {
			...
			if (cur.row + 1 < row && !visited[cur.row + 1][cur.column]) {
				...
				queue.offer(new Cell(cur.row + 1, cur.column, matrix[cur.row + 1][cur.column]));
			}
			if (cur.column + 1 < column && !visited[cur.row][cur.column + 1]) {
				visited[cur.column][cur.column + 1] = true;
				queue.offer(new Cell(cur.row, cur.column + 1, matrix[cur.row][cur.column + 1]));
			}
		}
