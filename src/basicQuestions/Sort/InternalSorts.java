 ...
 * sorting algorithms
 * 
 * Bucket sort Bubble sort Insertion sort Selection sort Heapsort Mergesort
 * 
 * 
 * Q1. insertion sort vs merge sort? 1. merge sort O(n) quxiliary space
 * insertion sort is in-place sort ---> o(1) quxiliary space
 * 
 * merge sort i Python = 2.2 nlgn microseconds insertion sort in Python = 0.2
 * n*n microseconds insertion sort in C = 0.01 n*n microseconds
 * 
 * @author Thomas
 *
 */
public class InternalSorts {
	...
	/**
	 * Let us compute the worst-time complexity of the insertion sort. In sorting
	 * the most expensive part is a comparison of two elements. Surely that is a
	 * dominant factor in the running time. We will calculate the number of
	 * comparisons of an array of N elements: we need 0 comparisons to insert the
	 * first element we need 1 comparison to insert the second element we need 2
	 * comparisons to insert the third element ... we need (N-1) comparisons (at
	 * most) to insert the last element Totally, 1 + 2 + 3 + ... + (N-1) = O(n2) The
	 * worst-case runtime complexity is O(n2).What is the best-case runtime
	 * complexity? O(n).
	 * 
	 * 
	 * note: binary insertion sort, save compare time, not saving swap times.
	 ...
	 * array (an array without first and second elements) and swap it with the third
