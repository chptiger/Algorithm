package Algorithm.Sort;

/**
 * Given a integer dictionary A of unknown size, where the numbers in the
 * dictionary are sorted in ascending order, determine if a given target integer
 * T is in the dictionary. Return the index of T in A, return -1 if T is not in
 * A.
 * 
 * Assumptions dictionary A is not null dictionary.get(i) will return
 * null(Java)/INT_MIN(C++) if index i is out of bounds
 * 
 * Examples A = {1, 2, 5, 9,......}, T = 5, return 2 A = {1, 2, 5, 9, 12,
 * ......}, T = 7, return -1
 * 
 * @author Thomas
 *
 */

/*
 * interface Dictionary { public Integer get(int index); }
 */

public class SearchInUnknownSizedSortedArray_13 {
	// public static int searchInUnknownSizedSortedArray(int[] arr, int T) {
	// if( arr == null ) return -1;
	// int index = 0;
	// while (dictionary[i] != null) {
	//
	// }
	// return 1;
	// }
}
