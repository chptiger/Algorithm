 ...
 * node in the tree. Examples 5 / \ 3 8 / \ 1 4 and 5 / \ 8 3 / \ 1 4 the two
 * binary trees are tweaked identical. How is the binary tree represented? We
 * use the level order traversal sequence with a special symbol "#" denoting the
 * null node. For Example: The sequence [1, 2, 3, #, #, 4] represents the
 * following binary tree: 1 / \ 2 3 / 4
 *
 * 
 */
public class TweakedIdenticalBinaryTrees_29 {
	...
	public static boolean tweakedIdenticalBinaryTrees(TreeNode one, TreeNode two) {
		...
		return (tweakedIdenticalBinaryTrees(one.left, two.left) && tweakedIdenticalBinaryTrees(one.right, two.right))
				|| (tweakedIdenticalBinaryTrees(one.left, two.right)
						&& tweakedIdenticalBinaryTrees(one.right, two.left));
	}
}
