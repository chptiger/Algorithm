...
/**
 * REFS:  
 * http://blog.csdn.net/fightforyourdream/article/details/16843303 �?�试大总结之二：Java�?�定�?�试中的二�?�树题目
 * http://blog.csdn.net/luckyxiaoqiang/article/details/7518888          轻�?��?�定�?�试中的二�?�树题目 
 * http://www.cnblogs.com/Jax/archive/2009/12/28/1633691.html           算法大全（3） 二�?�树 
 *  
 * 1. 求二�?�树中的节点个数: getNodeNumRec（递归），getNodeNum（迭代） 
 * 2. 求二�?�树的深度: getDepthRec（递归），getDepth  
 * 3. �?�?�??历，中�?�??历，�?��?�??历: preorderTraversalRec, preorderTraversal, inorderTraversalRec, postorderTraversalRec 
 * (https://en.wikipedia.org/wiki/Tree_traversal#Pre-order_2) 
 * 4.分层�??历二�?�树（按层次从上往下，从左往�?�）: levelTraversal, levelTraversalRec（递归解法） 
 * 5. 将二�?�查找树�?�为有�?的�?��?�链表: convertBST2DLLRec, convertBST2DLL 
 * 6. 求二�?�树第K层的节点个数：getNodeNumKthLevelRec, getNodeNumKthLevel
 * 7. 求二�?�树中�?��?节点的个数：getNodeNumLeafRec, getNodeNumLeaf 
 * 8. 判断两棵二�?�树是�?�相�?�的树：isSameRec, isSame
 * 9. 判断二�?�树是�?是平衡二�?�树：isAVLRec 
 * 10. 求二�?�树的镜�?（破�??和�?破�??原�?�的树两�?情况）：
 *     mirrorRec, mirrorCopyRec
 *     mirror, mirrorCopy 
 * 10.1 判断两个树是�?�互相镜�?：isMirrorRec isMirror
 * 11. 求二�?�树中两个节点的最低公共祖先节点：
 *      LAC        求解最�?公共祖先, 使用list�?�存储path.
 *      LCABstRec  递归求解BST树.
 *      LCARec     递归算法 .
 * 12. 求二�?�树中节点的最大�?离：getMaxDistanceRec 
 * 13. 由�?�?�??历�?列和中�?�??历�?列�?建二�?�树：rebuildBinaryTreeRec
 * 14. 判断二�?�树是�?是完全二�?�树：isCompleteBinaryTree, isCompleteBinaryTreeRec
 * 15. 找出二�?�树中最长连续�?串(�?�全部往左的连续节点，或是全部往�?�的连续节点）findLongest
 */  
...
//        System.out.println(findLongest2(r1));
    ...
    /*
     * �?�以用 level LevelOrderTraversal �?�实现，我们用一个dummyNode�?�分隔�?�?�的层，这样�?��?�计算出实际的depth.
     *      1  
           / \  
          2   3  
         / \   \  
    ...
    */  
     ...
     *  从左到�?�的�?��? 与从�?�到左的�?�?的逆�?是一样的，所以就简�?�喽�? 哈哈
     *  用�?�外一个栈进行翻转�?��?�喽 
     */ 
    ...
    /** 
     *  分层�??历二�?�树（递归） 
     *  很少有人会用递归去�?�level traversal 
     *  基本�?想是用一个大的ArrayList，里�?�包�?�了�?一层的ArrayList。 
 ...
 */
    ...
    /*
     * 10.1. 判断两个树是�?�互相镜�? Iterator �?�法
     *  (1) 根必须�?�时为空，或是�?�时�?为空
     * 
     * 如果根�?为空:
     * traversal 整个树，判断它们是�?是镜�?，�?次都按照�??�?��?�traversal  
     * (1). 当�?节点的值相等
     * (2). 当�?节点的左�?�节点�?镜�?，
     ...
     *         4
     *   这里最大�?离定义为2，4的�?离，为3.      
     * 求二�?�树中节点的最大�?离 �?�二�?�树中相�?最远的两个节点之间的�?离。 (distance / diameter) 
     * 递归解法：
     * 返回值设计：
     * 返回1. 深度， 2. 当�?树的最长�?离  
     * (1) 计算左�?树的深度，�?��?树深度，左�?树独立的链�?�长度，�?��?树独立的链�?�长度
     * (2) 最大长度为三者之最：
     *    a. 通过根节点的链，为左�?�深度+2
     *    b. 左�?树独立链
     *    c. �?��?树独立链。
    ...
    /*
     * 14. 判断二�?�树是�?是完全二�?�树：isCompleteBinaryTree, isCompleteBinaryTreeRec
     * 进行level traversal, 一旦�?�到一个节点的左节点为空，�?��?�的节点的�?节点都必须为空。而且�?应该有下一行，其实就是队列中所有的
     * 元素都�?应该�?有�?元素。
     * */
     ...
     * 
     *    我们�?�以分解为：
     *    CompleteBinary Tree 的�?�件是：
     *    1. 左�?��?树�?�为Perfect binary tree, 并且两者Height相�?�
     *    2. 左�?树为CompleteBinaryTree, �?��?树为Perfect binary tree，并且两者Height差1
     *    3. 左�?树为Perfect Binary Tree,�?��?树为CompleteBinaryTree, 并且Height 相�?�
     *    
     *    Base �?�件：
     *    (1) root = null: 为perfect & complete BinaryTree, Height -1;
     *    
     *    而 Perfect Binary Tree的�?�件：
     *    左�?��?树�?�为Perfect Binary Tree,并且Height 相�?�。
     * */
    ...
    public static ReturnBinaryTree isCompleteBinaryTreeRecHelp(TreeNode root) {
        ...
        /*
         * CompleteBinary Tree 的�?�件是：
         *    1. 左�?��?树�?�为Perfect binary tree, 并且两者Height相�?�(其实就是本树是perfect tree)
         *    2. 左�?树为CompleteBinaryTree, �?��?树为Perfect binary tree，并且两者Height差1
         *    3. 左�?树为Perfect Binary Tree,�?��?树为CompleteBinaryTree, 并且Height 相�?�
         * */
