...
public class StringPermutation_Advanced2 {
	...
	public void test_allPermutations() {
		...
		Assert.assertEquals(new ArrayList<>(Arrays.asList("abc", "acb", "bac", "bca", "cba", "cab")),
				permutations("abc"));
	}
	...
	private static ArrayList<String> permutations(String input) {
		...
		if (StringUtil.isDuplicate(input.toCharArray())) {
			 ...
			 * permutationHelper(input.toCharArray(), new boolean[input.length()], 0,
			 * result, sb);
			 */
		}
	...
	/**
	 * sb.deleteCharAt(sb.length()-1); // remove the appended char Q: If we remove
	 * it in result.add(new String(sb)), what will happen? A: sb is only removed in
	 * the last level operation. It's the recursion function, in the backtracking
	 * process, sb is not changed.
	 * 
	 * Q: why we need remove it? A: sb will add new character everytime. we need
	 * only keep size is input.length; append new char gradually and finally put it
	 * into the result in the backtracking, gradually removed it and add other char.
	 * 
	 */
