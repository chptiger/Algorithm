...
/**
 * Description Given a string, find the longest substring without any repeating
 * characters and return the length of it. The input string is guaranteed to be
 * not null.
 * 
 * For example, the longest substring without repeating letters for "bcdfbd" is
 * "bcdf", we should return 4 in this case.
 * 
 * When we move the R border(fast), we add information to the set. When we move
 * the L border(slow), we delete information from the set.
 * 
	...
	public static int uniqueSubstring(char[] input) {
		...
		while (fast < input.length) {
			if (container.contains(input[fast])) {
				// removing from the first letter, until the duplicated char was removed, to do
				// the next one.
