...
//	keep there are max k 0 in the slide window.
	private static int longestSubarray(char[] input, int k) {
		...
		/*there are two cases for right-hand side and left-hand side.
		 * fast pointer:
		 * 
		 * if (input[fast] == '0') {  // next is 0
				k--;
				result++;
			} else {				// next is 1
				result++;
		...
		*/
		while (fast < input.length) {
			...
			while (k < 0) { // the index of slow could be 1 or 0, we need keep moving until removing last 0, to keep the fixed 0 length in the sliding window
				if(input[slow] == '0') {
					k++;
				} 
				result--;
