 ...
 * 
 * Assumptions The given input string is not null. The characters to be removed
 * is given by another string, it is guranteed to be not null.
 * 
 * Examples input = "abcd", t = "ab", delete all instances of 'a' and 'b', the
 * result is "cd”.
 * 
 * two pointer run in the same direction
 * 
 * Initialization (2个挡板，3个区域，同向而行) slow = 0; all letters to the left-hand side
 * of slow(not including fast) are all processed letters that should not be
 * removed; fast = 0; fast is the current index to move. all letters in [slow,
 * fast] are all area that we do not care (fast, size - 1] unknown area to
 * explore
 * 
 * @author xx65
 *
