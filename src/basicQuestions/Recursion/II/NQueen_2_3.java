...
public class NQueen_2_3 {
	...
	/**
	 * cur: 1->3->5 in the row 0, first queen is in the column 1 in the row 1,
	 * second queen is in the column 3 in the row 2, thrid queen is in the column 5
	 */
	...
	// Queen could threaten each other if they are in the same row, same column and
	// diagonal
	...
	public static boolean valid(List<Integer> cur, int column) {
		...
		for (int r = 0; r < row; r++) { // i is the row of previous Queens
			if (cur.get(r) == column // to check if the new Queen in the same column of the previous one
					|| Math.abs(cur.get(r) - column) == (row - r) // to check if the new Queen and the previous one in
																	// the diagonal
			) {
				return false;
			}
