...
/**
 * given a binary tree, find the node with the max different in the total num
 * descendants in its left and right subtree
 * 
 * 5 / \ 9 12 / \ \ 2 3 14
 * 
 * node 12, diff is 1
...
public class FindNodeWithTheMaxDiffInLeftRight_5_3 {
	...
	public int numNodesLeft(TreeNode root) {
		...
		int[] diff = new int[1]; // use array to keep pass by the value of the max value. if we declare a basic
									// type like int type, it's only working in one recrsion level
		diff[0] = -1;
