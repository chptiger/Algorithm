...
/*	Using fix length slide window with size target.length() to solve this question
    Minus method was used in the solution. I could use 0 to identify the substring(all characters in the array).
	fast pointer is the right-hand side of window, slow pointer is the left-hand side window
	At the beginning, the fast pointer <= target.length(); fast++ and do not need to consider the left-hand side
	When the fast pointer > target.length(), we need consider both side of the window.
	For fast pointer(right-hand side of the slide widow): check if the next character is in target and how many count number of that character we have.
	For slow pointer(left-hand side of the slide window): check the current index character is in the target and how many count number of that character we have.
	
	initial value container a:2, b:1, c:1; type_of_letters_to_match = 3(no duplicate character)
...
*/
	...
	private String anagramSubstring(char[] input, char[] target) {
		...
		while (fast < input.length) {
			// right side; to treat the new element if the element was contained in the target string, removing counting number both in HashMap and type_of_letters_to_match
			...
			// left side; to treat the left side element if the element was contained in the target string, adding counting number both in HashMap and type_of_letters_to_match
