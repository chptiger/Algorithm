...
//		triangle.add(m4);
	...
	// from top to down. always keep an array that keep minimum value: sums[j] = (sums[j]<sums[j-1]?sums[j]:sums[j-1]) + triangle.get(i).get(j); 
	...
	public static int minimumTotal_(LinkedList<LinkedList<Integer>> triangle) {  
	    ...
	    {  
	        ...
	        for(int j=i-1;j>=1;j--)  
	        {  
	            sums[j] = (sums[j]<sums[j-1]?sums[j]:sums[j-1]) + triangle.get(i).get(j);  // between first and last one of every row.
	        }  
