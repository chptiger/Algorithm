		...
		for( int i=1; i<=m;i++){
			for( int j=i; j <= n ; j++){// n-m+i 保证后面元素可以分成 m-i 段
				if( j > i){// segments i, elements j;
					//第i段包含arr[j]
					tmp[i][j] = tmp[i][j-1]+arr[j-1];// last segments includes arr[j];  array starts from index 0, arr[j-1] is the jth element
					...
					tmp[i][j] = tmp[i-1][j-1]+arr[j-1];// when i==j, all elements add together as result. every element is a segment
			}
		}
		
		int sum = 0;
		for( int j = m; j<=n;j++){
			if( sum < tmp[m][j]) sum = tmp[m][j];
		}
